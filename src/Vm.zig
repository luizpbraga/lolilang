...
pub fn currentFrame(vm: *Vm) *Frame {
    vm.fm = &vm.frames[vm.frames_index - 1];
    return vm.fm;
}
...
pub fn popFrame(vm: *Vm) *Frame {
    vm.frames_index -= 1;
    vm.fm = &vm.frames[vm.frames_index];
    return vm.fm;
}
...
pub fn run(vm: *Vm) anyerror!void {
    ...
    while (vm.fm.ip + 1 < fm.instructions().len) : (vm.cursor += 1) {
        vm.fm.ip += 1;
        ip = @ptrCast(&fm.ip);
        vm.instructions = fm.instructions();
        op = @enumFromInt(vm.instructions[@intCast(vm.fm.ip)]);
        ...
        switch (op) {
            .set_range => {
                ...
                var end = vm.pop();
                if (end != .integer) {
                    end = .{ .integer = std.math.maxInt(i32) };
                }
            ...
            .to_range => {
                const pos = std.mem.readInt(u16, vm.instructions[ip.* + 1 ..][0..2], .big);
                fm.ip += 2;
                const value = vm.pop();
            ...
            .array => {
                const num_elements = std.mem.readInt(u16, vm.instructions[ip.* + 1 ..][0..2], .big);
                fm.ip += 2;
            ...
            .destruct => {
                const num_elements = std.mem.readInt(u8, vm.instructions[ip.* + 1 ..][0..1], .big);
                const scope: code.Opcode = @enumFromInt(std.mem.readInt(u8, vm.instructions[ip.* + 2 ..][0..1], .big));
                const index = std.mem.readInt(u16, vm.instructions[ip.* + 3 ..][0..2], .big);
                fm.ip += 4;
            ...
            .method_set => {
                const field_pos = std.mem.readInt(u16, vm.instructions[ip.* + 1 ..][0..2], .big);
                fm.ip += 2;
                const value = vm.pop();
                var left = vm.pop();
            ...
            .setgv => {
                const global_index = std.mem.readInt(u16, vm.instructions[ip.* + 1 ..][0..2], .big);
                fm.ip += 2;
                const value = vm.pop();
            ...
            .setlv => {
                const local_index = std.mem.readInt(u8, vm.instructions[ip.* + 1 ..][0..1], .big);
                fm.ip += 1;
                const value = vm.pop();
            ...
            .getbf => {
                const builtin_index = std.mem.readInt(u8, vm.instructions[ip.* + 1 ..][0..1], .big);
                fm.ip += 1;
                const builtin = builtins.builtin_functions[builtin_index];
            ...
            .constant => {
                const const_index = std.mem.readInt(u16, vm.instructions[ip.* + 1 ..][0..2], .big);
                fm.ip += 2;
            ...
            .getfree => {
                const free_index = std.mem.readInt(u8, vm.instructions[ip.* + 1 ..][0..1], .big);
                fm.ip += 1;
                const closure = fm.cl;
            ...
            .closure => {
                const index = std.mem.readInt(u16, vm.instructions[ip.* + 1 ..][0..2], .big);
                const num_free = std.mem.readInt(u8, vm.instructions[ip.* + 3 ..][0..1], .big);
                fm.ip += 3;
            ...
            .type => {
                const decl_len = std.mem.readInt(u8, vm.instructions[ip.* + 1 ..][0..1], .big);
                const fields_len = std.mem.readInt(u8, vm.instructions[ip.* + 2 ..][0..1], .big);
                const type_type = std.mem.readInt(u8, vm.instructions[ip.* + 3 ..][0..1], .big);
            ...
            .instance => {
                const fields_number = std.mem.readInt(u8, vm.instructions[ip.* + 1 ..][0..1], .big);
                fm.ip += 1;
            ...
            .class => {
                ...
                var fields: std.StringHashMap(Value) = .init(vm.allocator);
                errdefer fields.deinit();

                var tag: []const u8 = undefined;
                for (start_index..end_index) |i| {
                    const value = vm.stack[i];
                    if (value == .tag) {
                        tag = value.tag;
                        continue;
                    }
                    try fields.put(tag, value);
                }

                vm.sp = vm.sp - num_elements;
                // std.debug.print("{any}", .{vm.stack[end_index + 1]});
                const obj = try memory.allocateObject(vm, .{ .type = .{
                    .fields = fields,
                } });
                errdefer vm.allocator.destroy(obj);
            ...
            .call => {
                const args_number = std.mem.readInt(u8, vm.instructions[ip.* + 1 ..][0..1], .big);
                fm.ip += 1;

                const caller_value = vm.stack[vm.sp - 1 - args_number];
                // por alguma razao, usar f{} na chamada de uma funcao faz com que caller_value apponte
                // pro primeiro campo da struct, e nao pra ela mesmo.
                // o caso correto seria usar args_number == 0, e portanto
                // vm.stack[vm.sp - 1] seria a instancia
                ...
                switch (caller_value) {
                    ...
                    .builtin => |bui| {
                        ...
                        return vm.newError("else: Caller is not a Function: got {s}", .{caller_value.name()});
                    },
                }
            },
        }
